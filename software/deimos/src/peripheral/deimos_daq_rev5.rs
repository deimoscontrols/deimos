...
py_peripheral_methods!(DeimosDaqRev5);
...
impl Peripheral for DeimosDaqRev5 {
    ...
    fn standard_calcs(&self, name: &str) -> BTreeMap<String, Box<dyn Calc>> {
        ...
        {
            for (n, i) in (1..).zip(milliamp_4_20_range) {
                let input_name = format!("{name}.ain{i}");
...

        {
            ...
            let voltage_calc = Affine::new(input_name, 1.0 / 25.7, 0.0);
            calcs.insert(voltage_calc_name, voltage_calc);
        }

        {
            ...
            let voltage_calc = Affine::new(input_name, 1.0 / (1.0 + 49.4e3 / 75.0), 0.0);
            calcs.insert(voltage_calc_name, voltage_calc);
        }
