...
pub struct DeimosDaqRev6 {
    pub serial_number: u64,
}

py_peripheral_methods!(DeimosDaqRev6);
...
impl Peripheral for DeimosDaqRev6 {
    ...
    fn standard_calcs(&self, name: &str) -> BTreeMap<String, Box<dyn Calc>> {
...

        {
            ...
            let module_bus_current = Affine::new(format!("{name}.ain0"), 1.0 / (0.006 * 50.0), 0.0);
        ...
        // Cold junction RTD is also board temp
        {
            ...
            let filtered_calc = Butter2::new(format!("{temperature_calc_name}.temperature_K"), 1.0);
        ...
        // Variety of raw voltages with different gains
        {
            ...
            let voltage_calc = Affine::new(input_name, 1.0, 0.0);
            calcs.insert(voltage_calc_name, voltage_calc);
        }
...

        {
            ...
            let voltage_calc = Affine::new(input_name, 6.0, 0.0);
            calcs.insert(voltage_calc_name, voltage_calc);
        }
...

        {
            ...
            let voltage_calc = InverseAffine::new(input_name, 25.7, 1.024);
            calcs.insert(voltage_calc_name, voltage_calc);
        }

        {
            ...
            let voltage_calc = InverseAffine::new(input_name, 1.0 + 49.4e3 / 75.0, 1.024);
            calcs.insert(voltage_calc_name, voltage_calc);
        }
