...
fn main() {
    ...
    controller.add_peripheral("p2", Box::new(DeimosDaqRev6 { serial_number: 5 }));
    controller.add_peripheral("p3", Box::new(DeimosDaqRev6 { serial_number: 6 }));

    // Set up database dispatchers
    let timescale_dispatcher: Box<dyn Dispatcher> = Box::new(TimescaleDbDispatcher::new(
        "postgres",
        "192.168.8.231:5432", // Unix socket interface; TCP works as well
        "postgres",
        "POSTGRES_PW_2",
    ...
    controller.set_peripheral_input_source("p3.pwm0_duty", "duty.y");
    controller.set_peripheral_input_source("p3.pwm0_freq", "freq0.y");
    controller.set_peripheral_input_source("p3.dac0", "dac0.y");
    controller.set_peripheral_input_source("p3.dac1", "dac1.y");
